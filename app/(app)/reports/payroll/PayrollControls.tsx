...
/**
 * Scope + export controls for the payroll register. In batch mode (`batchId`
 * set) the scope is fixed and only the download links show; otherwise a date
 * range picker navigates the server-rendered preview. CSV/PDF are plain download
 * links to the ADMIN-gated report API.
 */
export function PayrollControls({
  ...
  const base = "/api/reports/payroll";
  const query = batchId ? `batchId=${encodeURIComponent(batchId)}` : `from=${fromDate}&to=${toDate}`;

  const apply = () => {
    router.push(`/reports/payroll?from=${fromDate}&to=${toDate}` as Route);
  };
    ...
    <div className="bg-surface border border-outline-variant rounded-xl p-5 flex flex-wrap items-end gap-4">
      {batchId ? (
        <div className="flex flex-col gap-1">
          <span className="text-label-mono uppercase tracking-widest text-on-surface-variant/70">Scope</span>
          <span className="font-mono text-body-sm text-on-surface">Batch {batchId}</span>
        </div>
      ) : (
        <>
