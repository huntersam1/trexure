...
/**
 * Period picker + export controls for the Yield Attribution report (#161 P6).
 * Applying a range navigates (re-rendering the preview); CSV/PDF are plain
 * download links to the ADMIN-gated report API, which streams the file.
 */
export function YieldReportControls({
  from,
  to,
  downloadQuery,
}: {
  ...
  const apply = () => {
    router.push(`/reports/yield?from=${fromDate}&to=${toDate}` as Route);
  };

  const base = "/api/reports/yield";
