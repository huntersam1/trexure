...
const { requirePlatformAdmin, listTenantsWithCounts, recordAudit } = vi.hoisted(() => ({
...
describe("GET /api/admin/tenants", () => {
  beforeEach(() => { requirePlatformAdmin.mockReset(); listTenantsWithCounts.mockReset(); recordAudit.mockClear(); });

  it("returns 403 problem+json for a non-platform-admin (C1: tenant enumeration is operator-only)", async () => {
  ...
  it("returns tenants with user counts for a platform admin and audits the read", async () => {
    requirePlatformAdmin.mockResolvedValue({ id: "u_admin", role: "ADMIN", tenantId: "t_hq", username: "admin", isPlatformAdmin: true });
